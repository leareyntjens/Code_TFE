...
def transcript_uncertainties(uncer_params, config):
    ...
    if "gwp_op_original" not in config:
        config["gwp_op_original"] = config['all_data']['Resources']['gwp_op'].copy()

    # Appliquer les incertitudes une seule fois
    for tech in config['all_data']['Resources'].index:
        gwp_key = 'gwp_' + tech.lower()
        if gwp_key in up:
            print(f"Avant modification : {tech} gwp_op = {config['gwp_op_original'][tech]}")
            print(f"up['{gwp_key}'] = {up[gwp_key]}")

            # Appliquer une seule fois sans cumul
            config['all_data']['Resources'].loc[tech, 'gwp_op'] = config['gwp_op_original'][tech] * (1. + up[gwp_key])
