...
def run_ESTD_UQ(sample):
    ...
    config = es.load_config(config_fn=os.path.join(path_rheia,'CASES','MEMOIRE','config_ref_UQ.yaml'))
...
def transcript_uncertainties(uncer_params, config):
    ...
    df = pd.read_csv( os.path.join(path_rheia,'CASES','MEMOIRE','design_space_ref.csv'), header=None)
