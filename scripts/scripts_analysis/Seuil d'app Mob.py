...
familles_modifiees = ["electrique"]  # Exemple : modifier seulement le GWP des fuel_cell
...
while gwp_val >= gwp_min:
    run_name = f"{i}_MOB_{familles_modifiees[0]}_gwp{int(round(gwp_val))}"
    ...
    for fam, total_f in fam_sums.items():
        if not found_appearance[fam] and total_f > 1:
            print(f"✅ Apparition de {fam} détectée à GWP = {gwp_val:.2f}")
            found_appearance[fam] = True

        if not found_dominance[fam] and fam == fam_max:
            print(f"🏆 {fam} devient dominante avec f_total = {total_f:.4f}")
            found_dominance[fam] = True
        else:
