...
param cost_limit >= 0;    # [M€/year] maximum cost allowed.
param lim_gwp_op >= 0;    # [ktCO2-eq./year] maximum gwp emissions allowed.
...
subject to totalGWP_calc:
	TotalGWP =  sum {j in TECHNOLOGIES} (GWP_constr [j] / lifetime [j]);
	#Just construction : sum {j in TECHNOLOGIES} (GWP_constr [j] / lifetime [j]);
...
# [Eq. 2.34]  constraint to reduce the total cost subject to Minimum_cost_reduction :
subject to Minimum_cost_reduction :
	TotalCost <= cost_limit;
	
# [Eq. 2.34]  constraint to reduce the GWP subject to Minimum_gwp_reduction :
subject to Minimum_GWP :
	sum {i in RESOURCES} GWP_op [i] <= lim_gwp_op;
